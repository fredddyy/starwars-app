<template>
  <div class="container">
    ...
    <!-- for the open crawl -->
    <div id="board">
      <div class="content" :class="{ 'content ani': animated }">
        <p id="title" v-if="selectedMovie">
  ...
  <section class="table-section">
    <div>
      <select
        id="selectButton"
        v-model="selectedMovieInd"
        :disabled="animated"
        @change="updateSelection"
      >
        <option disabled value="">Select A Movie</option>
        <option v-for="(film, index) in filmList" :value="index">
          {{ film.title }}
        </option></select
    ...
    <div class="table-container">
      <table>
        <thead>
          <tr>
            <th @click="sortArrayByName(this.data2)">Name</th>
            <th>Gender</th>
            <th @click="sortArrayByHeight(this.data2)">Height</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(i, ind) in data2" class="">
            <td>{{ i.name }}</td>
            <td v-if="i.gender == male">M</td>
            <td v-else-if="i.gender == female">F</td>
            <td v-else>{{ i.gender }}</td>
            <td>{{ i.height }}</td>
          </tr>
          <tr>
...
export default {
  ...
  methods: {
    async getFilms() {
      try {
        ...
        console.log(res.data.results), (this.movie = res.data.results);
      } catch {
    ...
    // update character list with new movie selection[index]
    updateSelection() {
      this.animated = false;
      this.animate();
      console.log(this.animated);
      this.selectedMovie = this.movie[this.selectedMovieInd];
      this.characters = this.selectedMovie.characters;
      this.character = [];
    },
    ...
    filter(g) {
      this.data2 = this.character;
      let array = this.character.filter((per) => per.gender === g.target.value);
      this.data2 = array;
    },
  },

  computed: {
    // sorts movie list by release date
    filmList() {
      return this.movie.sort(
        (a, b) => new Date(a.release_date) - new Date(b.release_date)
      );
    },

    loader() {
      if (this.data2.length > 1) return true;
    },
  ...
  watch: {
    // loop through characters array, fetch data and save them another arrar(character)
    async characters(newChar) {
      if (newChar.length > 0) {
        this.totalHeight = [];
        let char = this.characters;
        for (const i of char) {
          try {
            const res = await axios(i);
            this.character.push(res.data);
            this.data2 = this.character;
          } catch {}
        }
      }
...
.content.ani {
  animation: scroll 40s linear 10s;
}
